...
*函数名    ：WriteCardData
*函数功能  ：往卡片的某个块地址写数据
*函数参数  ：u8 addr,u8 *data,u8 *picc_id    
*函数返回值：u8
*函数描述  ：块的绝对地址0~63  注意不写到控制块中
             要写的数据  一定是16个字节
***********************************************/
u8 WriteCardData(u8 addr,u8 *data,u8 *picc_id)
{
...
*函数描述  ：块的绝对地址0~63  注意不写到控制块中
             读到的数据读到哪里
***********************************************/
u8 ReadCardData(u8 addr,u8 *data,u8 *picc_id)
{
...
u8 Enroll_PICC(void)
{
	u8 picc_Enroll_flag = 1;
	u8 picc_buff[16] = {0};    //存储识别到的卡号的
	u8 rest_buff[16] = {0};
	u8 cont = 0;
	
	
	/*往1号块中写入他的ID号*/
	if(WriteCardData(1,picc_buff,picc_buff) != MI_OK)
	{
	...
	if(i==10)
	{
		...
		WriteCardData(1,rest_buff,picc_buff);
...
u8 PICC_clock(void)
{
	u8 picc_buff[4] = {0};    //存储识别到的卡号的
	u8 val_buff[16] = {0};    //存储读出来的内容
	
	/*读取1号块获取内容*/
	if(ReadCardData(1,val_buff,picc_buff) != MI_OK)
	{
		return 2;
	}
	/*ID与内容对比*/
